...
from modeling_roberta_debug import RobertaForMultipleChoiceSVO
...
MODEL_CLASSES = {
    ...
    "roberta": (RobertaConfig, RobertaForMultipleChoiceSVO, RobertaTokenizer),
}
...
def collate_fn(batch):
    ...
    """
    all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_query_len, all_opt_len, all_svo_ids, all_lens, all_doc_len = map(torch.stack, zip(*batch))
    # all_input_ids, all_attention_mask, all_token_type_ids, all_lens, all_labels = map(torch.stack, zip(*batch))
    max_len = max(all_lens).item()
    all_input_ids = all_input_ids[:, :max_len]
    all_attention_mask = all_input_mask[:, :max_len]
    all_token_type_ids = all_segment_ids[:, :max_len]
    return all_input_ids, all_attention_mask, all_token_type_ids, all_label_ids, all_query_len, all_opt_len, all_svo_ids, all_doc_len
...
def train(args, train_dataset, model, tokenizer):
    ...
    def evaluate_model(train_preds, train_label_ids, tb_writer, args, model, tokenizer, best_steps, best_dev_acc):
        ...
        # wandb.log({"loss":results["eval_loss"], "train_acc": train_acc, "eval_acc": results['eval_acc']})
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                inputs = {
                    ...
                    "token_type_ids": batch[2]
                    if args.model_type in ["bert", "xlnet"]
                    else None,  # roberta don't use segment_ids
                    "labels": batch[3],
                    "query_len": batch[4], 
                    "opt_len": batch[5],
                    "svo_ids": batch[6],
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    cached_features_file = os.path.join(
        args.data_dir,
        "cached_{}_{}_{}_{}_SVO_ori_s".format(
            cached_mode,
    ...
    all_query_len = torch.tensor(select_field(features, "query_len"), dtype=torch.long)
    all_opt_len = torch.tensor(select_field(features, "opt_len"), dtype=torch.long)
    all_svo_ids = torch.tensor(select_field(features, "svo_ids"), dtype=torch.long)
    all_label_ids = torch.tensor([f.label for f in features], dtype=torch.long)
    # all_masked_ids = torch.tensor([f.masked_option for f in features], dtype=torch.long)
    # all_attention_mask = torch.tensor([f.masked_attention for f in features], dtype=torch.long)

    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_query_len, all_opt_len, all_svo_ids) # , all_masked_ids, all_attention_mask)
    return dataset
...
def main():
    ...
    parser.add_argument("--max_svo_len",
                        default=10,
                        type=int,
                        help="The maximum length of the svo triplets")
    parser.add_argument("--svo_weight",
                        default=1.0,
                        type=float,
                        help="weight of svo_loss")
    ...
    tokenizer.add_special_tokens({'additional_special_tokens':['<neg>', '<pos>', '<edu>']})
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint, max_doc_len=args.max_doc_len, max_query_len=args.max_query_len, max_option_len=args.max_option_len,svo_weight=args.svo_weight)
            model.to(args.device)
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        ...
        model = model_class.from_pretrained(checkpoint_dir, max_doc_len=args.max_doc_len, max_query_len=args.max_query_len, max_option_len=args.max_option_len, svo_weight=args.svo_weight)
        model.to(args.device)
